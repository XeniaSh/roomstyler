...
public abstract class BrowserFactory {
	...
	public static RemoteWebDriver setUp(final Browsers type) {
		...
		case CHROME:
			...
			URL myTestURLChrome = ClassLoader.getSystemResource("chromedriver.exe");
			try {
				myFile = new File(myTestURLChrome.toURI());
		...
		case FIREFOX:
			DesiredCapabilities cpl = DesiredCapabilities.firefox();
			FirefoxOptions opt = new FirefoxOptions();
			cpl.setCapability(FirefoxOptions.FIREFOX_OPTIONS, opt);
            URL myTestURLFirefox = ClassLoader.getSystemResource("geckodriver.exe");
            try {
                myFile = new File(myTestURLFirefox.toURI());
            } catch (URISyntaxException e1) {
                Logger.getInstance().debug(e1.getMessage());
            }
            System.setProperty("webdriver.gecko.driver", myFile.getAbsolutePath());
            driver = new FirefoxDriver(cpl);
			break;
